...
import { night, settingsTerrarium, temperature, wet, down_big, open_settings } from '@/shared/assets/imageAssets';
                    ...
                    <div className={s.right_side_wrapper}>
                        ...
                        <div className={s.pageTitle}>
                            <div className={s.leftSide}>
                                <h1>{terrarium.length > 0 ? terrarium[1]?.name.length > 40 ? terrarium[1]?.name.slice(0, 40) + '...' : terrarium[1]?.name : '-'}</h1>
                                {/* <img src={pencil}></img> */}
                            </div>
                        ...
                        <h3 onClick={() => setIsSettingsOpen(prev => !prev)} className={s.settings_button}>Текущие настройки <img className={`${s.settings_icon} ${isSettingsOpen ? s.rotated : ''}`} src={open_settings}></img></h3>
                        <div className={`${s.toggle_settings} ${isSettingsOpen ? s.open : ''}`}>
                            <div className={s.statistic}>
                                <div className={s.statistic_row_main}>
                                    <img src={temperature} alt="Temperature Icon" />
                                    <p>Воздух</p>
                                ...
                                <div className={s.statistic_row}>
                                    <p>Нижний угол</p>
                                    <p>{terrarium.length > 0 ? `${terrarium[1]?.settings.temperature_cold_night} - ${terrarium[1]?.settings.temperature_cold_day}` : '-'} °C</p>
                                </div>
                                <div className={s.statistic_row}>
                                    <p>Верхний угол</p>
                                    <p>{terrarium.length > 0 ? `${terrarium[1]?.settings.temperature_hot_night} - ${terrarium[1]?.settings.temperature_hot_day}` : '-'} °C</p>
                                </div>
                            </div>
                            <div className={s.statistic}>
                                <div className={s.statistic_row_main}>
